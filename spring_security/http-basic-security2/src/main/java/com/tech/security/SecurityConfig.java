...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(AuthenticationManagerBuilder auth) throws Exception {
		...
		auth
			...
			.withUser("admin").password(passwordEncoderEncoder().encode("admin123"))
			.roles("ADMIN").authorities("Access_Test1","Access_Test2")
			.and()
			.withUser("rakesh").password(passwordEncoderEncoder().encode("rakesh123"))
			.roles("USER")
			.and()
			.withUser("manager").password(passwordEncoderEncoder().encode("manager123"))
